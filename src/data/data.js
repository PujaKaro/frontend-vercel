...
export const products = [
  ...
  // Function to get suggested pujas based on a puja
  export const getSuggestedPujas = (pujaId) => {
    // Find the current puja
    const currentPuja = pujaServices.find(puja => puja.id === parseInt(pujaId));
    
    if (!currentPuja) return [];
    
    // Find pujas with similar occasions
    const similarPujas = pujaServices
      .filter(puja => 
        puja.id !== currentPuja.id && 
        puja.occasions.some(occasion => currentPuja.occasions.includes(occasion))
      )
      .sort((a, b) => {
        // Count matching occasions
        const aMatches = a.occasions.filter(occasion => currentPuja.occasions.includes(occasion)).length;
        const bMatches = b.occasions.filter(occasion => currentPuja.occasions.includes(occasion)).length;
